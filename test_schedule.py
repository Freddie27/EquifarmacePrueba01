...
def scrap():
    ...
    print(sumIDP)
    array = []
    FarmaciaLocal="Farmacia RedFarma"
    contadorWhile =0
    ...
    ActualizarP=0
    ProductoN=0
    while ContadorItems <8:
        if(contadorPagina==2):
            break
        page = urlopen("https://www.redfarma.cl/productos/obtener-productos-ajax?cantidad=8&familia=MEDICAMENTOS&pagina="+str(contadorPagina)+'').read()
        soup = BeautifulSoup(page, 'lxml')
        properties=json.loads(page)['productos']
        for item in properties:
            ...
            Nombreespacio = Nombre.replace(" ","-")
            Nombreslash = Nombreespacio.replace("/","")
            Nombreporcentaje = Nombreslash.replace("%","")
            Codigo= item['ArticuloCodigo']
            listado="www.redfarma.cl/productos/"+Nombreporcentaje+"/"+Codigo
            if ContadorItems==8:
                ContadorItems=0
                contadorPagina+=1
            ...
            try:
                ...
                Precio= (item['PrecioNormal'])
                NuevoDatoPrecio(Precio)
                Componente= (item['Activo'])
                NuevoDatoComponente(Componente)
                NuevoDatoFarmacia(FarmaciaLocal)
                NuevoDatoLink(listado)
                ProductoActualizado(sumIDP)
        ...
        print("www.redfarma.cl/productos/"+Nombreporcentaje+"/"+Codigo)
        print("---------------------------")
        """
    meta_data=MetaData()
    productos = Table('producto', meta_data,
...
schedule.every().day.at("17:38").do(scrap)
